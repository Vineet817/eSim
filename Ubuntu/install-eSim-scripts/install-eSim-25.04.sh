...
{
    echo "Installing KiCad via Snap (PPA unsupported on Ubuntu 24.10+)..."
...
function installDependency
{
 ...
 echo "Installing Pip............................"
sudo apt install -y python3-pip python3-venv xz-utils

echo "Installing Python packages inside virtual environment"

# Ensure pip points to venv
...
# Detect installed KiCad major version
KICAD_VERSION=$(kicad --version | grep -oE '[0-9]+' | head -1)
KICAD_CONFIG="$HOME/.config/kicad/${KICAD_VERSION}.0"

echo "Detected KiCad version: $KICAD_VERSION"
echo "Using KiCad config directory: $KICAD_CONFIG"

# Create KiCad config directory if missing
mkdir -p "$KICAD_CONFIG"

# Copy symbol table for eSim custom symbols
cp kicadLibrary/template/sym-lib-table "$KICAD_CONFIG/"
echo "symbol table copied successfully"

# Copy KiCad symbols (system-wide)
if [ -d /usr/share/kicad/symbols ]; then
    ...
    echo "WARNING: /usr/share/kicad/symbols not found"
    echo "KiCad symbols may need manual configuration"
fi

# Cleanup extracted library
rm -rf kicadLibrary
